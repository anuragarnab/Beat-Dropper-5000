...
//
// - Now with P4 Denormal fix :) - Has been removed by Team 3
...
void init_3band_state(EQSTATE* es, int lowfreq, int highfreq, int mixfreq)
{
 ...
 // memset(es,0,sizeof(EQSTATE)); 
  ...
  es->lf = 2 * sinf(M_PI * ((float)lowfreq / (float)mixfreq));
  es->hf = 2 * sinf(M_PI * ((float)highfreq / (float)mixfreq));
}
